    ...
    <ScrollView contentContainerStyle={{ paddingBottom: Platform.OS === 'web' ? 150 : 120 }}>
      ...
      {playlists.length === 0 ? (
        <View style={styles.empty}>
          ...
          <Text style={[styles.emptyTitle, { color: colors.foreground }]}>No Playlists</Text>
          <Text style={[styles.emptyText, { color: colors.mutedForeground }]}>
        ...
        <View style={styles.playlistGrid}>
          {playlists.map(pl => {
            ...
            return (
              <TouchableOpacity
                key={pl.id}
                style={[styles.playlistCard, { backgroundColor: colors.card, borderColor: colors.border }]}
              ...
              >
                <View style={[styles.playlistArt, { backgroundColor: colors.muted }]}>
                  <Ionicons name="musical-notes" size={32} color={colors.primary} />
                </View>
                <Text style={[styles.playlistName, { color: colors.foreground }]} numberOfLines={1}>{pl.name}</Text>
                <Text style={[styles.playlistCount, { color: colors.mutedForeground }]}>{pl.trackIds.length} tracks</Text>
              </TouchableOpacity>
            );
          })}
        </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
            ...
            <View style={styles.modalBtns}>
              <TouchableOpacity onPress={() => setShowCreate(false)} style={[styles.modalBtn, { backgroundColor: colors.muted }]}>
...
function StatsTab() {
  ...
  return (
    <ScrollView contentContainerStyle={{ paddingHorizontal: 20, paddingBottom: Platform.OS === 'web' ? 150 : 120, gap: 20, paddingTop: 20 }}>
      <View style={[styles.statCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
        <Text style={[styles.statLabel, { color: colors.mutedForeground }]}>Total Listening Time</Text>
      ...
      <View style={[styles.statCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
        ...
        <Text style={[styles.statValue, { color: colors.accent }]}>{todayMins} min</Text>
      </View>

      <View style={[styles.statCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
        <Text style={[styles.statLabel, { color: colors.mutedForeground }]}>Library Size</Text>
        <Text style={[styles.statValue, { color: colors.foreground }]}>{tracks.length} tracks</Text>
      </View>

      {topTracks.length > 0 && (
        <View style={[styles.statSection, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <Text style={[styles.statSectionTitle, { color: colors.foreground }]}>Top Tracks</Text>
          {topTracks.map((t, i) => (
            <View key={t.id} style={styles.statRow}>
              <Text style={[styles.statRank, { color: colors.primary }]}>#{i + 1}</Text>
              <View style={{ flex: 1 }}>
                <Text style={[styles.statRowTitle, { color: colors.foreground }]} numberOfLines={1}>{t.title}</Text>
                <Text style={[styles.statRowSub, { color: colors.mutedForeground }]}>{t.artist}</Text>
              </View>
              <Text style={[styles.statRowCount, { color: colors.mutedForeground }]}>{t.playCount} plays</Text>
            </View>
          ))}
        </View>
      )}
      ...
      {topArtists.length > 0 && (
        <View style={[styles.statSection, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <Text style={[styles.statSectionTitle, { color: colors.foreground }]}>Top Artists</Text>
          {topArtists.map(([artist, count], i) => (
            <View key={artist} style={styles.statRow}>
              <Text style={[styles.statRank, { color: colors.accent }]}>#{i + 1}</Text>
              <Text style={[styles.statRowTitle, { color: colors.foreground, flex: 1 }]} numberOfLines={1}>{artist}</Text>
              <Text style={[styles.statRowCount, { color: colors.mutedForeground }]}>{count} plays</Text>
            </View>
          ))}
        </View>
      )}
      ...
      {tracks.length === 0 && (
        <View style={styles.empty}>
          <Ionicons name="stats-chart-outline" size={56} color={colors.mutedForeground} />
...
const styles = StyleSheet.create({
  ...
  statLabel: { fontSize: 12, fontWeight: '600', letterSpacing: 1, textTransform: 'uppercase' },
